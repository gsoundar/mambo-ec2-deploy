...
public class NFSFileSystem extends FileSystem {
  ...
  private NFSFileSystemStore store;

  private LRUCache<String, FileHandle> handleCache;
  ...
  public final static Log LOG = LogFactory.getLog(NFSFileSystem.class);
  ...
  public void initialize(URI uri, Configuration conf) throws IOException {
    ...
    this.handleCache = new LRUCache<String, FileHandle>(FILE_HANDLE_CACHE_SIZE);
  ...
  private void initializeInternal(URI uri, Configuration conf)
    ...
    store = new NFSFileSystemStore(uri, conf);
  ...
  public NFSFileSystem(NFSFileSystemStore store) {
    this.store = store;
  }

  public NFSFileSystem(URI uri, Configuration conf) throws IOException {
    initialize(uri, conf);
  }
  ...
  public boolean rename(Path src, Path dst) throws IOException {
    ...
    FileHandle srcHandle = getAndVerifyFileHandle(src.getParent());
    FileHandle dstHandle = getAndVerifyFileHandle(dst.getParent());
    ...
    RENAME3Response rename3Response =
        store.rename(srcHandle, src.getName(), dstHandle, dst.getName(),
            store.getCredentials());
    int status = rename3Response.getStatus();
  ...
  public FileStatus[] listStatus(Path f) throws FileNotFoundException,
    ...
    for (Path path : listSubPaths(f)) {
      fileStatusList.add(getFileStatus(path));
    }
