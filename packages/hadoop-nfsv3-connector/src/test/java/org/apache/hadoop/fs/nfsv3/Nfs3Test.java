...
public class Nfs3Test {
  ...
  public void NullConfiguration() throws Exception {
    ...
    // Null configuration
    {
      NFSFileSystemStore temp = new NFSFileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), null);
      try {
    ...
    
    {
      ...
      conf.setBoolean("mambo.test", true);
      conf.set("mambo.test.mountpath", MOUNT_PATH);
      NFSFileSystemStore temp = new NFSFileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
    ...
    
    {
      ...
      errored = false;
      NFSFileSystemStore temp = null;
      try {
        temp = new NFSFileSystemStore(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
        temp.initialize(new URI("nfs://localhost:" + nfsServerPort + "/"), conf);
        temp.shutdown();
      } catch(IOException exception) {
        errored = true;
  ...
  public void Mount() throws Exception {
    RpcClientTcp client = new RpcClientTcp("localhost", nfsServerPort);
  ...
  public void ConnectionPoolTest() throws Exception {
    RpcClientTcp client = new RpcClientTcp("localhost", nfsServerPort, 16);
