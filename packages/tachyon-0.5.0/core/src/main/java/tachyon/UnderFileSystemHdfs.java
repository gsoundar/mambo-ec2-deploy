...
public class UnderFileSystemHdfs extends UnderFileSystem {
  ...
  private UnderFileSystemHdfs(String fsDefaultName, Object conf) {
	LOG.info("UnderFileSystemHdfs.getClient: path: "+ fsDefaultName + " conf: "+ conf);
    try {
      ...
      if (fsDefaultName.startsWith(glusterfsPrefix)) {
    	...
    	if (tConf.get("fs.nfs.mountdir") == null) {
    		LOG.info("get UNDERFS_NFS_MOUNT_DIR: "+ CommonConf.get().UNDERFS_NFS_MOUNT_DIR);
        	tConf.set("fs.nfs.mountdir", CommonConf.get().UNDERFS_NFS_MOUNT_DIR);
    	}
    	LOG.info("Set tConf: mountdir"+ tConf.get("fs.nfs.mountdir"));
